...
public abstract class Box
{
    ...
    // -HOnly try to open HTTP sendsms interface; if it fails, only warn about that, do not exit. (smsbox only)
    // --tryhttp
    ...
    // -dStart process as daemon (detached from a current shell session). Note: Process will change CWD (Current working directory) to /, therefore you should ensure that all paths to binary/config/config-includes are absolute instead of relative.
    // --daemonize
    ...
    // -PStart watcher process. This process watch a child process and if child process crashed will restart them automatically.
    // --parachute
