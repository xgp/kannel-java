 ...
 * @author garth
 */
public class SendsmsUserConfiguration extends Configuration {
  ...
  /** username Name for the user/account. */
  ...
  /** password Password for the user (see HTTP interface, below) */
  ...
  /**
   * user-deny-ip As other deny/allow IP lists, but for this user (i.e. this user is not allowed to
   * do the SMS push HTTP request from other IPs than allowed ones). If not set, there is no
   * limitations.
   */
  private String[] userDenyIp;

  public String[] getUserDenyIp() {
    return this.userDenyIp;
  }

  public void setUserDenyIp(String[] userDenyIp) {
    this.userDenyIp = userDenyIp;
  }

  /** user-allow-IP */
  private String[] userAllowIp;

  public String[] getUserAllowIp() {
    return this.userAllowIp;
  }

  public void setUserAllowIp(String[] userAllowIp) {
    this.userAllowIp = userAllowIp;
  }

  /** forced-smsc Force SMSC ID as a 'string' (linked to SMS routing, see 'smsc' groups) */
  private String forcedSmsc;

  public String getForcedSmsc() {
    return this.forcedSmsc;
  }

  public void setForcedSmsc(String forcedSmsc) {
    this.forcedSmsc = forcedSmsc;
  }

  /**
   * default-smsc If no SMSC ID is given with the send-sms request (see below), use this one. No
  ...
  /** default-sender This number is set as sender if not set by from get/post parameter */
  ...
  /** faked-sender As in 'sms-service' groups */
  ...
  /**
   * allowed-prefix A list of phone number prefixes which are accepted to be sent using this
   * username. Multiple entries are separated with semicolon (';'). For example, "040;050" prevents
   * sending of any SMS message with prefix of 040 or 050 through this SMSC. If denied-prefix is
   * unset, only this numbers are allowed. If set, number are allowed if present in allowed or not
   * in denied list.
   */
  ...
  /**
   * denied-prefix A list of phone number prefixes which are NOT accepted to be sent using this
   * username.
   */
  ...
  /** black-list As white-list, but SMS messages from these numbers are automatically rejected. */
  ...
  /** dlr-url URL to be fetched if a dlr-mask CGI parameter is present. */
  private URL dlrUrl;
  ...
  /**
   * allowed-prefix-regex A set of phone numbers which are accepted to be sent using this username.
  ...
  /**
   * denied-prefix-regex A set of phone numbers which may not send using this username. See section
  ...
  /**
   * white-list-regex Defines a set of accepted destinations of SMS messages. If a destination of an
   * SMS message is not in this list, any message received from the HTTP interface is rejected. See
  ...
  /**
   * black-list-regex As white-list-regex, but SMS messages originating from a number matching the
