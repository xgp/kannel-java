 ...
 * @author garth
 */
public class HttpKannelAdmin implements KannelAdmin {
  ...
  public void flushDLR() throws AdminException {
    try {
      ...
      if (response.contains("before trying to flush DLR queue"))
        throw new AdminException(response.trim());
    } catch (Exception e) {
  ...
  public void startSMSC(String smsc) throws AdminException {
    try {
      ...
      if (response.contains("Could not re-start smsc-id") || response.contains("SMSC id not given"))
        throw new AdminException(response.trim());
    } catch (Exception e) {
  ...
  public void addSMSC(String smsc) throws AdminException {
    try {
      ...
      query.append("?").append("smsc=").append(smsc);
      String response = getContent(getUrl(query.toString(), true));
      if (response.contains("Could not add smsc-id") || response.contains("SMSC id not given"))
        throw new AdminException(response.trim());
    } catch (Exception e) {
  ...
  public void removeSMSC(String smsc) throws AdminException {
    try {
      ...
      query.append("?").append("smsc=").append(smsc);
      String response = getContent(getUrl(query.toString(), true));
      if (response.contains("Could not remove smsc-id") || response.contains("SMSC id not given"))
        throw new AdminException(response.trim());
    } catch (Exception e) {
  ...
  /**
   * A command line client for testing. usage: HttpKannelAdmin <host> <password> <method>
   * <opt:param>
   */
