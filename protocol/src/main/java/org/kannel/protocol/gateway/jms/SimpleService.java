...
public class SimpleService implements MessageListener {
  ...
  public SimpleService(Properties props) throws NamingException, JMSException {
    this.properties = props;

    try {
      this.contenido = new Properties();
      this.contenido.load(
          new FileInputStream(new File(this.properties.getProperty(prop_ContentFile))));
    ...
    queueConnectionFactory =
        (QueueConnectionFactory)
            jndiContext.lookup(
                this.properties.getProperty(SimpleJMSTransport.prop_ConnectionFactoryName));

    inBoundTopic =
        (Queue)
            jndiContext.lookup(
                this.properties.getProperty(SimpleJMSTransport.prop_OutBoundTopicName));
    outBoundTopic =
        (Queue)
            jndiContext.lookup(
                this.properties.getProperty(SimpleJMSTransport.prop_InBoundTopicName));
      ...
      if (parMessage instanceof ObjectMessage) {
        ...
        if (this.contenido != null) {
          ...
          if (swap != null) {
            ...
            outOMessage.setObject(
                new SimpleMessage(mesg.getReceiver(), mesg.getSender(), "", swap));
