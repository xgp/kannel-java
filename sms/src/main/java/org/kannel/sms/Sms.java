...
public class Sms extends Msg {
  ...
  /**
   * username (or user) Username or account name. Must be username of the one 'sendsms-user' group
   * in the Kannel configuration, or results in 'Authorization failed' reply.
   */
  ...
  /**
   * password (or pass) Password associated with given username. Must match corresponding field in
   * the 'sendsms-user' group of the Kannel configuration, or 'Authorization failed' is returned.
   */
  ...
  /**
   * validity Optional. If given, Kannel will inform SMS Center that it should only try to send the
   * message for this many minutes. If the destination mobile is off other situation that it cannot
   * receive the sms, the smsc discards the message. Note: you must have your Kannel box time
  ...
  /**
   * deferred Optional. If given, the SMS center will postpone the message to be delivered at now
   * plus this many minutes. Note: you must have your Kannel box time synchronized with the SMS
   * Center.
   */
  ...
  /**
   * dlr-mask Optional. Request for delivery reports with the state of the sent message. The value
   * is a bit mask composed of: 1: Delivered to phone, 2: Non-Delivered to Phone, 4: Queued on SMSC,
   * 8: Delivered to SMSC, 16: Non-Delivered to SMSC. Must set dlr-url on sendsms-user group or use
   * the dlr-url CGI variable.
   */
  ...
  /**
   * dlr-url Optional. If dlr-mask is given, this is the url to be fetched. (Must be url-encoded)
   */
  private URL dlrUrl;
  ...
  /**
   * pid Optional. Sets the PID value. (See ETSI Documentation). Ex: SIM Toolkit messages would use
   * something like &pid=127&coding=1&alt-dcs=1&mclass=3
   */
  private Byte pid;
  ...
  /**
   * alt-dcs Optional. If unset, Kannel uses the alt-dcs defined on smsc configuration, or 0X per
   * default. If equals to 1, uses FX. If equals to 0, force 0X.
   */
  ...
  /** rpi Optional. Sets the Return Path Indicator (RPI) value. (See ETSI Documentation). */
  private Integer rpi;
